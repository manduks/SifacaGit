class SchoolPdf < Prawn::Document
  ...
  def folio
    cell_1 = make_cell(:content => "Factura ", :align => :center, :size => 10, :background_color => "F5A9A9")
    cell_2 = make_cell(:content => "No. de Folio:", :size => 10)
    cell_3 = make_cell(:content => "Fecha", :align => :center, :size => 10, :background_color => "F5A9A9")
  ...
  def client
    ...
    cell_7 = make_cell(:content => "#{Client.find(Invoice.find(@resume.invoice_id).client_id).street} ##{Client.find(Invoice.find(@resume.invoice_id).client_id).num_ext} #{Client.find(Invoice.find(@resume.invoice_id).client_id).suburb} #{Client.find(Invoice.find(@resume.invoice_id).client_id).cp} #{Client.find(Invoice.find(@resume.invoice_id).client_id).township} #{Client.find(Invoice.find(@resume.invoice_id).client_id).state}", :width => 410)
  ...
  def article
    ...
    cell_3 = make_cell(:content => "Subtotal", :background_color => "F5A9A9", :width => 100)
    cell_4 = make_cell(:content => "#{number_to_currency(@resume.subtotal, :unit => "$")}", :align => :right, :width => 90)
    cell_5 = make_cell(:content => "I.V.A Tasa 16 %", :background_color => "F5A9A9")
    cell_6 = make_cell(:content => "#{number_to_currency(@resume.total - @resume.subtotal, :unit => "$")}", :align => :right)
    cell_7 = make_cell(:content => "Total a Pagar", :background_color => "F5A9A9")
  ...
  def student
    cell_1 = make_cell(:content => "Nombre del Alumno: ", :background_color => "F5A9A9")
    cell_2 = make_cell(:content => "#{Student.find(@invoice.student_id).name}", :width => 407)
  ...
  def inf
    cell_1 = make_cell(:content => "CURP del Alumno: ",:background_color => "F5A9A9", :width => 123)
    cell_2 = make_cell(:content => "#{Student.find(@invoice.student_id).curp}", :width => 170)
    cell_3 = make_cell(:content => "Grado Escolar: ", :background_color => "F5A9A9", :width => 110)
    cell_4 = make_cell(:content => "#{Student.find(@invoice.student_id).grade}", :width => 127)
