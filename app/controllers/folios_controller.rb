class FoliosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @folio.errors.empty? && @folio.save
        ...
        format.html { redirect_to clients_path, notice: 'El folio fue agregado satisfactoriamente.' }
  ...
  def update
    ...
    respond_to do |format|
      if @folio.errors.empty? && @folio.update_attributes(params[:folio])
        format.html { redirect_to folios_path, notice: 'El folio fue actualizado satisfactoriamente.' }
        format.json { head :no_content }
        format.js
      else
        format.html { redirect_to folios_path }##render action: "edit" }
        format.json { render json: @folio.errors, status: :unprocessable_entity }
        format.js
      end
    end
  ...
  def validateFolios(folio, params, action)
    ...
    if params[:initiation].empty?
      ...
      if action == 'update'
        if Integer(params[:initiation]) <= (Folio.find_max_by_user_id(current_user.id, folio.id) || 0)
          folio.errors.add 'initiation', 'Debe ser mayor que '+(Folio.find_max_by_user_id(current_user.id, folio.id) || 0).to_s
        end
      else
        if Integer(params[:initiation]) <= (Folio.find_max_by_user_id(current_user.id, nil) || 0)
